...
capture_ublox_config_file = 'capture_ublox_config.json'
...
def load_qerr_config():
    if not os.path.exists(capture_ublox_config_file):
        raise FileNotFoundError(f"{capture_ublox_config_file} does not exist."
                                f"\nPlease re-run this script with the help flag -h to see the option to create it.")
    with open(capture_ublox_config_file) as f:
        return json.load(f)
...
def make_json_config():
    ...
    if not os.path.exists(capture_ublox_config_file):
        with open(capture_ublox_config_file, 'w') as f:
